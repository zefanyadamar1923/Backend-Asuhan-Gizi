...
import { getAsuhanGiziRI, getSaveAsuhanGiziRI, saveAsuhanGiziRI } from '../services/formAsuhanGiziRIService';
import { IAsuhanGiziRI, IApiResponse } from '../@types';
...
export const getSaveAsuhanGiziRIController = async (req: Request, res: Response): Promise<void> => {
  try {
    const { no_reg } = req.params;
    if (!no_reg) {
      res.status(400).json({ success: false, message: 'no_reg tidak boleh kosong' } as IApiResponse);
    ...
    const data = await getSaveAsuhanGiziRI(no_reg as string);
    if (data && data.length > 0) {
      res.status(200).json({ success: true, message: 'Data ditemukan', data } as IApiResponse);
    } else {
      res.status(404).json({ success: false, message: 'Data tidak ditemukan' } as IApiResponse);
    ...
    logger.error('Error in getSaveAsuhanGiziRIController', error);
    res.status(500).json({ success: false, message: 'Internal server error' } as IApiResponse);
  }
};

// SAVE RI (Anak & Dewasa)
export const saveAsuhanGiziRIController = async (req: Request, res: Response): Promise<void> => {
  try {
    const data: IAsuhanGiziRI = req.body;
    if (!data.vc_no_reg) {
    ...
    await saveAsuhanGiziRI(data);
    res.status(200).json({ success: true, message: 'Successfully saved Asuhan Gizi RI' } as IApiResponse);
  } catch (error) {
    logger.error('Error in saveAsuhanGiziRIController', error);
    res.status(500).json({ success: false, message: 'Internal server error' } as IApiResponse);
  }
};
